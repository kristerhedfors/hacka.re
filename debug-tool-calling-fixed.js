...
// Function to safely override ApiToolsService methods
function setupApiToolsServiceOverrides() {
    if (!window.ApiToolsService) {
        HackaReDebug.warn('Initialization', 'ApiToolsService not available, skipping method overrides');
        return false;
    }

    try {
        // Override formatToolsForOpenAI to log tool formatting
        if (typeof ApiToolsService.formatToolsForOpenAI === 'function') {
            ...
            HackaReDebug.warn('Tools', 'formatToolsForOpenAI method not found on ApiToolsService');
        }
        
        // Override getApiTools to log tool retrieval
        if (typeof ApiToolsService.getApiTools === 'function') {
            ...
            HackaReDebug.warn('Tools', 'getApiTools method not found on ApiToolsService');
        }
        
        // Override init to log initialization
        if (typeof ApiToolsService.init === 'function') {
            ...
            HackaReDebug.warn('Tools', 'init method not found on ApiToolsService');
        }
        
        // Override addApiTool to log tool addition
        if (typeof ApiToolsService.addApiTool === 'function') {
            ...
            HackaReDebug.warn('Tools', 'addApiTool method not found on ApiToolsService');
        }
        
        // Override removeApiTool to log tool removal
        if (typeof ApiToolsService.removeApiTool === 'function') {
            ...
            HackaReDebug.warn('Tools', 'removeApiTool method not found on ApiToolsService');
        }
        
        // Override processToolCalls to log tool execution
        if (typeof ApiToolsService.processToolCalls === 'function') {
            ...
            HackaReDebug.warn('Tools', 'processToolCalls method not found on ApiToolsService');
        }
        
        // Override executeToolCall to log individual tool execution
        if (typeof ApiToolsService.executeToolCall === 'function') {
            ...
            ApiToolsService.executeToolCall = async function(toolCall, apiKey) {
                ...
                try {
                    // Use direct mock response for testing instead of calling the original method
                    // This avoids the need to make actual API calls
                    ...
                    if (storedTools) {
                        try {
                            ...
                            HackaReDebug.error('Tool Execution', 'Error parsing stored tools', e);
                        }
                    }
                    ...
                    // Generate a mock response based on the tool type
                    let result;
                    
                    if (toolName === 'weather_lookup') {
                        result = {
                            location: args.location || "Unknown",
                            temperature: Math.round(15 + Math.random() * 15) + (args.units === "imperial" ? "°F" : "°C"),
                            conditions: ["Sunny", "Partly Cloudy", "Cloudy", "Rainy"][Math.floor(Math.random() * 4)],
                            humidity: Math.round(40 + Math.random() * 40) + "%",
                            wind: Math.round(5 + Math.random() * 20) + (args.units === "imperial" ? " mph" : " km/h"),
                        ...
                        try {
                            ...
                            result = {
                                ...
                                result: typeof calcResult === 'number' ? calcResult.toFixed(precision) : String(calcResult),
                                precision: precision
                            };
                        } catch (error) {
                            result = {
                    ...
                    HackaReDebug.log('Tool Execution', `Tool "${toolName}" execution result (mock)`, result);
                    return result;
                } catch (error) {
            ...
            HackaReDebug.warn('Tools', 'executeToolCall method not found on ApiToolsService');
        }
        
        return true;
    } catch (error) {
        HackaReDebug.error('Initialization', 'Error setting up ApiToolsService overrides', error);
        return false;
    }
}

// Override ApiService methods to log API interactions
function setupApiServiceOverrides() {
    if (!window.ApiService) {
        HackaReDebug.warn('Initialization', 'ApiService not available, skipping method overrides');
        return false;
    }

    try {
        // Override generateChatCompletion to log chat generation
        if (typeof ApiService.generateChatCompletion === 'function') {
            ...
            HackaReDebug.warn('API', 'generateChatCompletion method not found on ApiService');
        }
        
        // Override fetchAvailableModels to log model fetching
        if (typeof ApiService.fetchAvailableModels === 'function') {
            ...
            HackaReDebug.warn('API', 'fetchAvailableModels method not found on ApiService');
        }
        
        // Override modelSupportsToolCalling to log tool support checks
        if (typeof ApiService.modelSupportsToolCalling === 'function') {
            ...
            HackaReDebug.warn('API', 'modelSupportsToolCalling method not found on ApiService');
        }
        
        return true;
    } catch (error) {
        HackaReDebug.error('Initialization', 'Error setting up ApiService overrides', error);
        return false;
    }
}
...
window.addEventListener('load', function() {
    ...
    // Set up API service overrides
    setupApiServiceOverrides();
    
    // Set up API tools service overrides
    setupApiToolsServiceOverrides();
