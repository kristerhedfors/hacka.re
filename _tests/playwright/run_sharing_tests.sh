...
for TEST_FILE in $SHARING_TESTS; do
    ...
    .venv/bin/python -m pytest "$TEST_FILE" $PYTEST_ARGS --browser $BROWSER $HEADLESS 2>&1 | tee -a "sharing_test_${TEST_NAME}.log"
...
_venv/bin/.venv/bin/python -m pytest $PYTEST_ARGS --browser $BROWSER $HEADLESS -k "$SHARING_FILTER" 2>&1 | tee -a sharing_test_combined.log
