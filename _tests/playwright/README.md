...
These tests use the `gpt-5-nano` model by default (configured in `conftest.py`) to minimize token costs while still testing real API functionality.
...
- We use `gpt-5-nano` from OpenAI during tests due to its lower token costs and faster response times.
