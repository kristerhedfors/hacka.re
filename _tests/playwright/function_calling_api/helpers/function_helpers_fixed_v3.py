...
function get_weather(location, unit = "celsius") {
  ...
  return {
    ...
    try:
        expect(function_name).to_have_value("get_weather", timeout=5000)
    except Exception as e:
        actual_value = function_name.input_value()
        print(f"ERROR: Function name field was not auto-populated correctly. Expected: 'get_weather', Actual: '{actual_value}'")
        # Take a screenshot for debugging
        page.screenshot(path="_tests/playwright/videos/function_name_not_populated.png")
        raise Exception(f"Function name field was not auto-populated correctly. Expected: 'get_weather', Actual: '{actual_value}'") from e
