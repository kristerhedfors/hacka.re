 ...
 * Connects directly to https://huggingface.co/mcp with Bearer token
 */
...
(function(global) {
    ...
    class HuggingFaceConnector extends global.BaseServiceConnector {
        ...
        async connectMCPServer(accessToken) {
            ...
            try {
                console.log('[HuggingFaceConnector] Connecting to Hugging Face MCP server:', this.config.mcpServerUrl);
                console.log('[HuggingFaceConnector] Using token authentication');

                await window.MCPClientService.connect(serverName, mcpConfig);
                this.mcpClient = window.MCPClientService;
                console.log('[HuggingFaceConnector] MCP server connected successfully');
                return true;
            } catch (error) {
                console.error('[HuggingFaceConnector] MCP connection error:', error);
                throw new Error(`Failed to connect to Hugging Face MCP server: ${error.message}`);
            }
        }
